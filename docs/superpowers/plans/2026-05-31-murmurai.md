...
| File | Responsibility |
|---|---|
| `src/settings.py` | Load/save settings dataclass to `%APPDATA%\NovaaAI\settings.json` |
...
| `novaaai.spec` | PyInstaller build spec |
...
APPDATA_DIR = Path(os.environ.get("APPDATA", Path.home())) / "Novaa AI"
...
MODEL_NAME = "small"
MODEL_DIR = Path(os.environ.get("APPDATA", Path.home())) / "Novaa AI" / "models"
...
MODEL_NAME = "small"
_MODEL_DIR = str(Path(os.environ.get("APPDATA", Path.home())) / "Novaa AI" / "models")
...
if __name__ == "__main__":
    app = NovaaAIWindow(on_toggle=lambda: print("toggle"), on_settings_save=lambda *a: None)
    app.mainloop()
```
...
```python
if __name__ == "__main__":
    app = NovaaAIWindow(on_toggle=lambda: None, on_settings_save=lambda h, l, s: print(h, l, s))
    app.mainloop()
```
...
def _set_start_on_login(enabled: bool) -> None:
    ...
    try:
        ...
        if enabled:
            winreg.SetValueEx(key, "Novaa AI", 0, winreg.REG_SZ, sys.executable)
        else:
            try:
                winreg.DeleteValue(key, "Novaa AI")
...
Expected: `dist/Novaa AI.exe` created.

- [ ] **Step 4: Test the executable**

Run `dist/Novaa AI.exe`. Verify the same smoke-test checklist from Task 12 Step 2.

- [ ] **Step 5: Commit**

```
git add novaaai.spec
